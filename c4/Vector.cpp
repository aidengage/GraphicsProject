...
// returns magnitude of a vector4
...
Vector4 Vector4::cross(Vector4 v) {
  float tempx = this->y*v.z - this->z*v.y;
  float tempy = this->z*v.x - this->x*v.z;
  float tempz = this->x*v.y - this->y*v.x;
  return Vector4(tempx, tempy, tempz, 1);
}
